...
if(read_base_func == FALSE) {
  ...
  ft.e = read.table(paste0(datadir_base_func,dirpath,ifelse(StdFollowUp,"Std","Org"), "Ft_Train.dat"),row.names = 1, header = TRUE)
  ft.e.S = read.table(paste0(datadir_base_func,dirpath,ifelse(StdFollowUp,"Std","Org"), "Sc_Train.dat"),row.names = 1, header = TRUE)
  TrainFt = ft.e[row.names(ft.e)%in%TrainSurv$case,]
  TrainSc = ft.e.S[row.names(ft.e.S)%in%TrainSurv$case, ]
  PKTS = read.table(paste0(datadir_base_func, dirpath, ifelse(StdFollowUp,"Std","Org"), "PKTS_Train.dat"), row.names = 1, header = TRUE)
  ValidFt = read.table(paste0(datadir_base_func, dirpath, ifelse(StdFollowUp,"Std","Org"), "Ft_Valid.dat"), row.names = 1, header = TRUE)
  ValidSc = read.table(paste0(datadir_base_func, dirpath, ifelse(StdFollowUp,"Std","Org"), "Sc_Valid.dat"), row.names = 1, header = TRUE)
  PKTS2 = read.table(paste0(datadir_base_func, dirpath, ifelse(StdFollowUp,"Std","Org"), "PKTS_Valid.dat"), row.names = 1, header = TRUE)
}
