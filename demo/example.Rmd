...
The first two data tables consists of labeled training and validation survival outcome (`event_ind`, `event_time`, `fu_time`) and baseline covariates (`cov_1`-`cov_3`).

```{r}
data_org[1:2]
```

The first two data tables consists of training and validation longitudinal encounters. `fu_time` is the follow-up time and `pred1`-`pred3` are the numbers of medical occurrences in each month (from baseline).
...
In order to analyze very large data, the `petler.base` function uses multi-core parallel computing by default (`n_core = 4`), and the core algorithms are written in efficient C++ code.
